"""進捗バー塗りのテーマ連動色（FR-025 / FR-028 / UC-011）。"""
...
def progress_label_color(percent: int, palette: UiThemePalette | None = None) -> str:
    ...
    if _relative_luminance(colors.card_bg) < 0.25:
        return PROGRESS_LABEL_LIGHT
    if clamp_progress(percent) >= PROGRESS_LABEL_WHITE_FROM:
        return PROGRESS_LABEL_LIGHT
    return colors.card_fg
