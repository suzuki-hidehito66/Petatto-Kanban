...
| [NFR-008](#nfr-008-単一ユーザー独立動作) | アーキテクチャ | 単一ユーザー・オフライン・独立 .exe 動作（FR-032 は任意例外） | Must | specified | M1 |
...
| 実装 | アーキテクチャ全体（コア機能はネットワークなし。FR-032 のみ任意の HTTP） |
...
| 2 | **コア機能**（カード CRUD・永続化・表示モード等）はネットワーク通信（HTTP/WebSocket 等）を行わない |
| 3 | ボード・設定データはローカル JSON ファイルのみに保存する。エラーログは [FR-031](./03-functional-requirements.md#fr-031-ローカルエラーログ) のローカルファイル |
| 4 | `.exe` 単体の起動で全 M1 **コア**機能が利用できる（オフライン含む） |
| 5 | マルチユーザー・共同編集機能を持たない |
| 6 | 例外: [FR-032](./03-functional-requirements.md#fr-032-github-issue-任意起票) は **オプトインかつトークンがあるときだけ**、標準ライブラリ `urllib` で GitHub Issues API に接続してよい。失敗してもコア機能は継続する |

**検証**
- `pyproject.toml` の runtime 依存が空であること
- HTTP 呼び出しは FR-032 の起票モジュールに限り、設定 OFF またはトークン無しでは実行されないこと（AC-NFR-008-01）
- 手動: ネットワーク切断状態で `.exe` のコア機能が動作すること（AC-NFR-008-02）。FR-032 が ON でも切断時は起票を諦め、アプリは止まらない
