...
`main` への取り込みは **`test` からの PR を squash マージ**する（1 リリース = 1 コミット）。マージ直後に `test` を `origin/main` へ `reset --hard` し、`--force-with-lease` で push する。詳細は [README.md ブランチ運用](../../README.md#ブランチ運用コントリビュータ向け) と [AGENTS.md](../../AGENTS.md)。
