...
| M1 | MVP | 単一ユーザー独立 .exe + **デスクトップモード** + 3 列カンバン + CRUD + 永続化 | **進行中** |
...
| 区分 | 内容 |
|------|------|
| 含む | FR-001〜009, FR-019, FR-021, FR-023, NFR-001〜008, NFR-011 |
| 含まない | ウィンドウモード（FR-018）、オーバーレイ（FR-020）、モード切替（FR-022）、FR-010 以降 |
