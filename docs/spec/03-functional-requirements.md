...
**説明**  
ツールバーの「+ カード」をクリックすると、即座に新しいカードを1枚追加する。

**制約**
- 初期タイトルは既定値 `新しいタスク`
- 新規カードはツールバー「+ カード」付近（ツールバー直下）に配置
...
**説明**  
タイトルラベルをダブルクリックしてインライン編集する。

**制約**
- タイトルラベル上の `<Double-Button-1>`、または FR-003 によるカード追加直後に `Entry` に切り替え
...
- ダブルクリック編集とドラッグ移動を両立する（短いクリック＋ドラッグは移動、ダブルクリックは編集）
...
**制約**
- カード枠およびタイトルラベルから左クリックドラッグで移動可能
- ドラッグ中はカードの `x`, `y` を更新
...
| 実装 | `src/petatto_kanban/app.py`, `due_date.py`, `due_date_picker.py` |
...
- 期限パネルの `<Double-Button-1>` でカード外のフロート期限編集パネルを開く（別ウィンドウは使用しない）
- フロートパネルに月間カレンダーと「期限なし」ボタンを提供
- カレンダー上の **当日** 日付ボタンは緑色で表示
- パネル外（カード・ツールバー・透過領域など）をクリックした場合は「閉じる」と同様にキャンセル（変更を保存しない）
...
**説明**  
ツールバー右上の **×ボタン** でアプリを終了する。終了前にボード・表示設定を自動保存する。
