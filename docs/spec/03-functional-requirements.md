...
- カード枠はタイトル表示領域より大きくなるよう、最小サイズ（幅 220px・高さ 88px）を下限とする
...
**制約**
- 初期タイトルは既定値 `新しいタスク`
- タイトル変更は FR-004（インライン編集）で行う
- 新規カードは既定位置 `(120, 120)` 基準にオフセット配置（既存カードと重なっても可）
- 入力ダイアログは表示しない
...
**制約**
- タイトルラベル上の `<Double-Button-1>` で `Entry` に切り替え
