...
When ユーザーが `<` 円をクリックする（ドラッグなし）
Then タイトル "新しいタスク" のカードがメニューパネル付近に即座に表示される
...
```gherkin
Given メニューパネルが表示されている
When ユーザーが `<` 円を2回クリックする（ドラッグなし）
Then タイトル "新しいタスク" のカードが2枚、メニューパネル付近に表示される
...
When ユーザーがメニューパネルホバー時の「×」をクリックする
Then アプリが終了する
