...
```gherkin
Given カラーテーマが Default である
And 進捗率 0% / 50% / 100% のカードがある
Then 進捗バーの塗りがそれぞれ明るい赤 / 黄 / 緑である
When ユーザーがカラーテーマを「ダーク」に変更して OK する
Then 進捗バーの塗りがテーマの progress_fill_low / mid / high になる
And 色相は赤 / 黄 / 緑のままである
And Default の塗り色とは明度が異なる
```

**テスト**: `test_progress.py`, `test_ui_theme.py` + 手動（Windows 11）
...
```gherkin
Given カラーテーマが適用されている
And 期限が本日のカードが表示されている
Then 期限パネルの背景がテーマの due_today_bg（黄基調）である
And 文字色がテーマの due_today_fg である
Given 期限が過去日のカードが表示されている
Then 期限パネルの背景がテーマの due_overdue_bg（赤基調）である
And 文字色がテーマの due_overdue_fg である
...
```gherkin
Given カラーテーマが適用されている
And ユーザーが期限編集のカレンダーパネルを開いている
Then 当日の日付ボタンがテーマの calendar_today_bg / calendar_today_fg で表示される
And 全テーマ共通の固定緑ではない
```
...
```gherkin
Given ユーザーが期限編集のカレンダーパネルを開いている
When ユーザーがいずれかの日付ボタンにマウスオーバーする
Then そのセルおよび隣接セルの位置・外寸は変わらない
And 当月に属さない空セルも日付ボタンと同じセル外寸を占める
When ユーザーがマウスを日付ボタンから外す
Then セルの位置・外寸はホバー前と同一である
```

**テスト**: `test_due_date.py` + 手動（Windows 11）
...
Given カラーテーマが適用されている
And ユーザーが期限編集のカレンダーパネルを開いている
When ユーザーが通常日（当日以外）の日付ボタンにマウスオーバーする
Then そのボタンの背景色がテーマの due_picker_day_hover_bg に変わる
And 非ホバー時の色とは異なる
When ユーザーが当日の日付ボタンにマウスオーバーする
Then そのボタンの背景色がテーマの calendar_today_hover_bg に変わる
And 非ホバー時の calendar_today_bg とは異なる
And 当日であることが識別できる
When ユーザーがマウスを日付ボタンから外す
Then 背景色はホバー前の色に戻る
```

**テスト**: `test_due_date.py`, `test_ui_theme.py` + 手動（Windows 11）
...
And カード上の期限パネルで当日はテーマの due_today_*（黄基調）・超過は due_overdue_*（赤基調）で表示される
And 進捗バーの塗りはテーマの progress_fill_*（0%≈赤 / 50%≈黄 / 100%≈緑）である
And カードの x / y 座標は変更されない
```
...
```gherkin
Given 設定ダイアログの「テーマ」タブを表示している
When ユーザーがカラーテーマを「フォレスト」に変更して OK する
Then カード背景が暗い緑系・タイトル文字が明るい色である
And カレンダー当日セルは forest の calendar_today_bg / calendar_today_fg である
When ユーザーがカラーテーマを「オーシャン」に変更して OK する
Then カード背景が暗い紺系・タイトル文字が明るい色である
When ユーザーがカラーテーマを「サンセット」に変更して OK する
Then カード背景が暗い暖色・タイトル文字が明るい色である
When ユーザーがカラーテーマを「ミッドナイト」に変更して OK する
Then カード背景が暗い紺系・タイトル文字が明るい色である
And カード・メニュー・期限パネルの配色は UC-011 の各テーマパレットに一致する
```

**テスト**: `test_ui_theme.py` + 手動（Windows 11）
...
## FR-032: GitHub Issue 任意起票（cancelled）

受け入れ基準は定義しない。診断は FR-031 のローカルログのみ。
