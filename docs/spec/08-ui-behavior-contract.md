...
| タイトル | — | 太字、`wraplength=110`（標準時）。`GROOVE` 枠の内側フレームで表示 |
...
| カード幅 | `CARD_MIN_WIDTH + 2 * CARD_FRAME_BORDER`（132） | 枠線込みの配置基準幅 |
...
| `PROGRESS_BAR_HEIGHT` | 11 px | 進捗バー |
| `CARD_FRAME_PAD` | 5 px | カード外枠内余白 |
