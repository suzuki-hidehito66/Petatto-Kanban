...
|------|-----|
| 関連 FR | FR-003, FR-023 |
| 実装 | `src/petatto_kanban/menu_panel.py`, `app.py` |
| UI 契約 | 旧「ツールバー」をメニューパネルに置換（M1） |

| 要素 | 操作 | 結果 |
|------|------|------|
| 円形 `<` パネル | — | 通常時は **円の中に `<` のみ**。周囲フレームは透過 |
| メニューパネル | マウスホバー | 左側に **＋**（カード追加）・**⚙**（設定）・**×**（終了）ボタンを展開表示 |
| **＋** ボタン | クリック→離す（ホバー時） | 円形 UI。`<ButtonRelease-1>` で反応。メニューパネル付近にカード即時追加（UC-004） |
| **⚙** ボタン | クリック→離す（ホバー時） | 円形 UI。`<ButtonRelease-1>` で反応。設定ダイアログ（FR-021, FR-024） |
| **×** ボタン | クリック→離す（ホバー時） | 円形 UI。`<ButtonRelease-1>` で反応。`_on_close()` — 保存後にアプリ終了 |
| メニューパネル | 左クリックドラッグ（`<` 上） | 画面上の位置を変更。座標は `settings.json` に永続化 |

**クリックの反応タイミング**
- **＋**・**⚙**・**×** は `<Button-1>` 押下のみでは動作しない。**離した** とき（`<ButtonRelease-1>`）に処理する
- 押下と離しが **同一ボタン上** で行われた場合のみ有効（ドラッグして外に出した場合はキャンセル）

**通常レイアウト（左 → 右、ホバー時）:** ＋ | ⚙ | × | **○<**

**配置:** デフォルトは画面右上（上端 16px・右端 16px マージン）。`<` 円の **右端** を基準（`anchor=NE`）に配置し、展開・収納時も `<` 円の位置は動かない。

**永続化:** `menu_panel_x`, `menu_panel_y`（左上座標。未設定時はデフォルト位置）
