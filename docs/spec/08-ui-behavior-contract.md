...
| **＋** ボタン | クリック（ホバー時） | 円形 UI。メニューパネル付近にカード即時追加（UC-004） |
| **⚙** ボタン | クリック（ホバー時） | 円形 UI。設定ダイアログ（FR-021, FR-024） |
| **×** ボタン | クリック（ホバー時） | 円形 UI。`_on_close()` — 保存後にアプリ終了 |
