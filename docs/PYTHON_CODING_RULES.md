...
├── system/           # OS 連携（自動起動・起動コマンド解決・ショートカット・グローバルホットキー）
...
| OS 連携 | `system/` | 自動起動（winreg）、起動コマンド解決、ショートカット正規化、グローバルホットキー |
| テスト | `tests/test_*.py` | モデル・ストレージ・設定・自動起動・ショートカットの単体テスト |
