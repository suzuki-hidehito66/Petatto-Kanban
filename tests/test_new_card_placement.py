...
def test_first_card_below_panel_right_aligned() -> None:
    ...
    assert x == panel.right - 132 - DEFAULT_NEW_CARD_INSET_X
...
def test_right_edge_uses_anchor_not_stale_width() -> None:
    ...
    x, _y = compute_new_card_position(panel=panel, card_width=132, stack_index=0)
    assert x == 1916 - 132 - DEFAULT_NEW_CARD_INSET_X
    assert x != panel.x + panel.width - 132


def test_stack_index_offsets_bottom_left() -> None:
    panel = _panel()
    x0, y0 = compute_new_card_position(panel=panel, card_width=132, stack_index=0)
    x1, y1 = compute_new_card_position(panel=panel, card_width=132, stack_index=1)
    assert x1 == x0 - DEFAULT_NEW_CARD_STACK_OFFSET_X
    assert y1 == y0 + DEFAULT_NEW_CARD_STACK_OFFSET_Y

    x2, y2 = compute_new_card_position(panel=panel, card_width=132, stack_index=2)
